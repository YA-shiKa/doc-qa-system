...
# ✅ SAFE MOCK INGESTION (NO ML / NO OCR)
async def _ingest_background(file_path: Path, doc_id: str) -> None:
    try:
        _documents[doc_id]["status"] = "processing"

        # 🔥 Mock processing (simulate work)
        _documents[doc_id]["total_sections"] = 5
        _documents[doc_id]["total_chunks"] = 20

        _documents[doc_id]["status"] = "ready"

        logger.info("mock_ingestion_complete", doc_id=doc_id)

    except Exception as e:
        _documents[doc_id]["status"] = "failed"
        _documents[doc_id]["error"] = str(e)
        logger.error("mock_ingestion_failed", doc_id=doc_id, error=str(e))
...
async def get_document(doc_id: str):
    ...
    return DocumentResponse(
        **{k: v for k, v in doc.items() if k not in ("file_path", "error")}
    )
...
async def list_documents():
    ...
    docs = [
        DocumentResponse(
            **{k: v for k, v in d.items() if k not in ("file_path", "error")}
        )
        for d in _documents.values()
    ]
...
@router.delete("/{doc_id}")
async def delete_document(doc_id: str):
    """Delete a document (mock removal)."""
    doc = _documents.get(doc_id)

    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")

    # ✅ Mock removal (no pipeline)
    removed_chunks = 0
